...
function get_form_access(id_string){

    $.ajax({
      url:'/asf/get_form_access/'+id_string+'/',
      type:'GET',
        async: false,
        data:{},
        dataType: 'json',
      success: function(data) {
        form_access_val = data;
      },
    });
    return form_access_val;
}
...
console.log(isOwner+'--'+dataObj);
 ...
 if ( isOwner ) {
    ...
    if (Can_View){
     ...
     if(Can_submit){
        d = get_form_access(dataObj);
        if (d == 0){
           newSubmission = '<a disabled=" " href="#"'+'  target="_blank" class="btn red btn-md" type="button">New Submission</a>';

        }
        if (d == 1){
           newSubmission = '<a  href="/asf/get-form'+'/'+dataObj+'/"'+'  target="_blank" class="btn red btn-md" type="button">New Submission</a>';
        ...
        // newSubmission = '<a disabled=" " href="/asf/get-form'+'/'+dataObj+'/"'+'  target="_blank" class="btn red btn-md" type="button">New Submission</a>';
