...
def check_form_edit_permissions(request, xform,user):
    frm_qry="select can_edit from vwrolewiseformpermission where xform_id="+str(xform.id)+" and user_id="+str(user.id)
    #frm_qry="select can_submit from vwrolewiseformpermission where xform_id=393 and user_id=85"
...
def check_form_submission_permissions(request, xform,user):
    frm_qry="select can_submit from vwrolewiseformpermission where xform_id="+str(xform.id)+" and user_id="+str(user.id)
    #frm_qry="select can_submit from vwrolewiseformpermission where xform_id=393 and user_id=85"
...
def check_submission_permissions(request, xform,user):
    ...
    #if request and (profile.require_auth or xform.require_auth or request.path == '/'+user.username+'/submission') and xform.user != request.user and not request.user.has_perm('report_xform', xform):
    if request and (profile.require_auth or xform.require_auth or request.path == '/'+user.username+'/submission') and xform.user != user.username and not check_form_submission_permissions(request, xform,user):
