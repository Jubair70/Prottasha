...
def survey_summary(request):
    ...
    for xform in xforms:
        ...
        pqry="SELECT  can_edit,can_view,can_submit,can_delete,can_setting	FROM vwrolewiseformpermission where  xform_id="+str(xform.id)+" and username='"+str(request.user)+"'"
        ...
        submission_date_query = "SELECT (json->>'_submission_time')::timestamp::date AS day_of_submission FROM logger_instance WHERE deleted_at is null and xform_id="+str(xform.id)+" and user_id in "+str(user_id)
        cursor.execute(submission_date_query)
        fetchVal = cursor.fetchall();
...
def instance_status_update(request):
    if request.method == 'POST':
        user_name= request.user.id
        status_msg = request.POST.get('note')
        status_name = request.POST.get('selected_status')
        instance_id = request.POST.get('instance_id')
        c = connection.cursor()
        try:
            c.execute("BEGIN")
            c.callproc("updateinstancestatus", [instance_id,user_name,status_name,status_msg]) 
            results = c.fetchall()
            c.execute("COMMIT")
        finally:
            c.close()
        ...
        for each in results:
            #print each[0]
            #print each[1]
            return HttpResponse(each[1]);

        #check whether user have permission to update status other raise permission denied
        #begin transaction
        #update status
