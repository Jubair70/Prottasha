...
def show_graph_def_get_json(request):
    datajson = getDatatable('select id, (select link_name from dashboard_navigation_bar where id=navigation_bar_id ) as Tab, datasource as Datasource, chart_object, element_id, navigation_bar_id,content_order,content_row from dashboard_generator order by navigation_bar_id,content_order,content_row')
