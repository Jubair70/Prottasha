...
def save_dashboard_style(request):
    ...
    if request.method == 'POST':
        ...
        dashboardGenerator.content_row = request.POST.get("content_row")
...
def show_graph_list(request):
    navigationBar = DashboardNavigationBar.objects.all()
    return render(request,"dashboard_tool/show_graph_list.html",{"navbar": navigationBar});



def show_graph_def_get_json(request,nav_id):
    datajson = getDatatable('select id, (select link_name from dashboard_navigation_bar where id=navigation_bar_id ) as Tab, datasource as Datasource, chart_object, element_id, navigation_bar_id,content_order,content_row from dashboard_generator where navigation_bar_id = %s order by content_order'%(nav_id))
...
def edit_graph_def(request, graph_id):
    graph_def_instance = DashboardGenerator.objects.get(pk=graph_id)
    # try:
    #     graph_def_name=graph_def_instance.id;
    #     graph_def_instance.delete()
    ...
    #     data=getAjaxMessage("danger", graph_def_del_message)
    context = {
        "graph_def":graph_def_instance,
    }
    return render(request, 'dashboard_tool/build_dashboard_edit.html', context)


def edit_dashboard_style(request):

    if request.method == 'POST':
        print "Test    ", request.POST.get("chart_type")
        
        dashboardGenerator = DashboardGenerator.objects.get(element_id=request.POST.get("element_id"))
        dashboardGenerator.datasource = request.POST.get("datasource")
        dashboardGenerator.save()
        return HttpResponse("Hello", status=200)
    else:
...
def add_navigation_bar(request):
    ...
    if request.method=="POST":
        ...
        if navigation_form.is_valid():
            ...
            navigation_form_instance.appearance = '{}'
            navigation_form_instance.save()
            navigation_form = DashboardNavigationBarForm()
            context = {
                "navigation_form": navigation_form
            }
...
def show_filtering_control_get_json(request):
    datajson = getDatatable('select id, (select link_name from dashboard_navigation_bar where id=navigation_bar_id ) as Tab, control_label as Label, control_type as Type, cascaded_by_id as "cascaded by",datasource as Datasource from dashboard_controls_generator order by id, Tab')
    return HttpResponse(datajson, content_type='application/json')
