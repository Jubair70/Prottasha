...
def victim_status(request,victim_tbl_id):
    if request.POST:
        ...
        updt_qry = "UPDATE public.asf_case SET assaign_to='" + str(assaign_to) + "',status='2' where id = " + str(case_id)
        __db_commit_query(updt_qry)
    ...
    if role == 'RSC Manager':
        qry += "where user_id =any (select user_id from usermodule_usermoduleprofile where rsc_name_id = (select rsc_name_id from usermodule_usermoduleprofile where user_id= "+str(user_id)+"))"
...
def victim_profile(request,victim_tbl_id):
    ...
    father_name = df.father_name.tolist()[0] if len(df.father_name.tolist()) and df.father_name.tolist()[0] is not None  else ''
    mother_name = df.mother_name.tolist()[0] if len(df.mother_name.tolist()) and df.mother_name.tolist()[0] is not None  else ''
    birth_country = df.birth_country.tolist()[0] if len(df.birth_country.tolist()) and df.birth_country.tolist()[0] is not None  else ''
    ...
    permanent_village = df.permanent_village.tolist()[0] if len(df.permanent_village.tolist()) and df.permanent_village.tolist()[0] is not None  else ''
    permanent_post_office = df.permanent_post_office.tolist()[0] if len(df.permanent_post_office.tolist()) and df.permanent_post_office.tolist()[0] is not None  else ''
...
def get_event_list(request):
    ...
    if role == 'Field Officer':
        q = "select ROW_NUMBER() OVER(ORDER BY id) AS serial_no,id,json->>'participant/total_participant' total_participant, date(json->>'event/event_start_time') event_date , (select field_name from geo_data where geocode = (json->>'geo/district')) district, (select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila from vw_groupevent where user_id = "+str(user_id)

    elif role == 'RSC Manager':
        q = "select ROW_NUMBER() OVER(ORDER BY id) AS serial_no,id,json->>'participant/total_participant' total_participant, date(json->>'event/event_start_time') event_date , (select field_name from geo_data where geocode = (json->>'geo/district')) district, (select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila from vw_groupevent where user_id = any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+"))"
    else:
...
def get_ipt_show_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'ipt_show_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    elif role == 'RSC Manager':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'ipt_show_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'ipt_show_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show') and deleted_at is null)select * from t"
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)
...
def ipt_profile(request,event_id):
    qry = " select id event_id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username ,(select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,json->>'geo/village' village ,json->>'participant/male_greater_equal_18' male_greater_equal_18 ,json->>'participant/male_less_18' male_less_18 ,json->>'participant/female_greater_equal_18' female_greater_equal_18 ,json->>'participant/female_less_18' female_less_18 ,json->>'participant/total_participant' total_participant , case when (json->>'participant/participant_category')::int = 1 then 'Aspirant Migrants' when (json->>'participant/participant_category')::int = 2 then 'Returnee' when (json->>'participant/participant_category')::int = 3 then 'Community Members' when (json->>'participant/participant_category')::int = 4 then 'Local Governments' when (json->>'participant/participant_category')::int = 5 then 'National Government' when (json->>'participant/participant_category')::int = 6 then 'Service Provider' when (json->>'participant/participant_category')::int = 99 then (json->>'participant/participant_category_other') end participant_category, json->>'remarks' remarks from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_ipt_show') and deleted_at is null and id = " + str(event_id)
    df = pandas.read_sql(qry,connection)
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{100,101}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def get_video_show_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'video_show_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    if role == 'RSC Manager':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'video_show_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'video_show_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show') and deleted_at is null)select * from t"
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)
...
def video_show_profile(request,event_id):
    qry = " select id event_id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username ,(select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,json->>'geo/village' village ,json->>'participant/male_greater_equal_18' male_greater_equal_18 ,json->>'participant/male_less_18' male_less_18 ,json->>'participant/female_greater_equal_18' female_greater_equal_18 ,json->>'participant/female_less_18' female_less_18 ,json->>'participant/total_participant' total_participant , case when (json->>'participant/participant_category')::int = 1 then 'Aspirant Migrants' when (json->>'participant/participant_category')::int = 2 then 'Returnee' when (json->>'participant/participant_category')::int = 3 then 'Community Members' when (json->>'participant/participant_category')::int = 4 then 'Local Governments' when (json->>'participant/participant_category')::int = 5 then 'National Government' when (json->>'participant/participant_category')::int = 6 then 'Service Provider' when (json->>'participant/participant_category')::int = 99 then (json->>'participant/participant_category_other') end participant_category, json->>'remarks' remarks from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show') and deleted_at is null and id = " + str(event_id)
    df = pandas.read_sql(qry,connection)
    event_id = df.event_id.tolist()[0] if len(df.event_id.tolist()) and df.event_id.tolist()[0] is not None  else ''
    date_created = df.date_created.tolist()[0] if len(df.date_created.tolist()) and df.date_created.tolist()[0] is not None  else ''
    submitted_by = df.username.tolist()[0] if len(df.username.tolist()) and df.username.tolist()[0] is not None  else ''
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{300,301}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def get_school_quiz_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'school_quiz_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    elif role == 'RSC Manager':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'school_quiz_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'school_quiz_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz') and deleted_at is null)select * from t"
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)
...
def school_quiz_profile(request,event_id):
    qry = " select id event_id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username ,(select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,json->>'geo/village' village ,json->>'participant/male_greater_equal_18' male_greater_equal_18 ,json->>'participant/male_less_18' male_less_18 ,json->>'participant/female_greater_equal_18' female_greater_equal_18 ,json->>'participant/female_less_18' female_less_18 ,json->>'participant/total_participant' total_participant , case when (json->>'participant/participant_category')::int = 1 then 'Aspirant Migrants' when (json->>'participant/participant_category')::int = 2 then 'Returnee' when (json->>'participant/participant_category')::int = 3 then 'Community Members' when (json->>'participant/participant_category')::int = 4 then 'Local Governments' when (json->>'participant/participant_category')::int = 5 then 'National Government' when (json->>'participant/participant_category')::int = 6 then 'Service Provider' when (json->>'participant/participant_category')::int = 99 then (json->>'participant/participant_category_other') end participant_category, json->>'remarks' remarks from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz') and deleted_at is null and id = " + str(event_id)
    df = pandas.read_sql(qry,connection)
    event_id = df.event_id.tolist()[0] if len(df.event_id.tolist()) and df.event_id.tolist()[0] is not None  else ''
    date_created = df.date_created.tolist()[0] if len(df.date_created.tolist()) and df.date_created.tolist()[0] is not None  else ''
    submitted_by = df.username.tolist()[0] if len(df.username.tolist()) and df.username.tolist()[0] is not None  else ''
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{400,401}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def get_tea_stall_meeting_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    elif role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_checklist') and deleted_at is null and (json->>'event_id') is not null limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_review') and deleted_at is null and (json->>'event_id') is not null limit 1) then 1 else 0 end review from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting') and deleted_at is null)select * from t"
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)
...
def tea_stall_meeting_profile(request,event_id):
    qry = " select id event_id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username ,(select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,json->>'geo/village' village ,json->>'participant/male_greater_equal_18' male_greater_equal_18 ,json->>'participant/male_less_18' male_less_18 ,json->>'participant/female_greater_equal_18' female_greater_equal_18 ,json->>'participant/female_less_18' female_less_18 ,json->>'participant/total_participant' total_participant , case when (json->>'participant/participant_category')::int = 1 then 'Aspirant Migrants' when (json->>'participant/participant_category')::int = 2 then 'Returnee' when (json->>'participant/participant_category')::int = 3 then 'Community Members' when (json->>'participant/participant_category')::int = 4 then 'Local Governments' when (json->>'participant/participant_category')::int = 5 then 'National Government' when (json->>'participant/participant_category')::int = 6 then 'Service Provider' when (json->>'participant/participant_category')::int = 99 then (json->>'participant/participant_category_other') end participant_category, json->>'remarks' remarks from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting') and deleted_at is null and id = " + str(event_id)
    df = pandas.read_sql(qry,connection)
    event_id = df.event_id.tolist()[0] if len(df.event_id.tolist()) and df.event_id.tolist()[0] is not None  else ''
    date_created = df.date_created.tolist()[0] if len(df.date_created.tolist()) and df.date_created.tolist()[0] is not None  else ''
    submitted_by = df.username.tolist()[0] if len(df.username.tolist()) and df.username.tolist()[0] is not None  else ''
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{200,201}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def call_center_form(request):
    ...
    if request.GET:
        ...
        instance_id = -1
    return render(request, 'asfmodule/call_center_support_form.html',{'username':username,'server_address':server_address,'form_id':form_id,'form_builder_server' : form_builder_server,'instance_id':instance_id})
...
def consultancy_matrix_form(request):
    ...
    if request.GET:
        ...
        instance_id = -1
    return render(request, 'asfmodule/consultancy_matrix_form.html',{'username':username,'server_address':server_address,'form_id':form_id,'form_builder_server' : form_builder_server,'instance_id':instance_id})
...
def catchment_tree_test(request,rsc_id):
    ...
    query = "WITH recursive t AS( SELECT id, field_name, field_parent_id FROM geo_data WHERE id IN ( SELECT (select id from geo_data where geocode = rsc_catchment_area.geo_id::text limit 1)geoid FROM PUBLIC.rsc_catchment_area WHERE rsc_id = "+str(rsc_id)+") UNION ALL SELECT geo_data.id, geo_data.field_name, geo_data.field_parent_id FROM geo_data, t WHERE t.field_parent_id = geo_data.id) SELECT DISTINCT id, field_name, field_parent_id FROM t ORDER BY id"
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
...
def get_check_nodes(rsc_id):
    query = "select (select id from geo_data where geocode = rsc_catchment_area.geo_id::text limit 1) geoid  from rsc_catchment_area where rsc_id = " + str(rsc_id) + ""
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
...
def catchment_data_insert(request):
    ...
    for each in result_set:
        if each:
            query = "INSERT INTO public.rsc_catchment_area (rsc_id, geo_id) VALUES(" + str(rsc_id) + ", (select geocode::int from geo_data where id=" + str(each) + "))"
