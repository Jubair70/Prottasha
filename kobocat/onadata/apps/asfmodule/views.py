...
def ipt_profile(request,event_id):
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{100,101,102}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def get_video_show_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'video_show_checklist') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show_review') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1) then 1 else 0 end review from logger_instance st where xform_id = (select id from logger_xform where id_string = 'event_video_show') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    if role == 'RSC Manager':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'video_show_checklist') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_video_show_review') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1) then 1 else 0 end review from logger_instance st where xform_id = (select id from logger_xform where id_string = 'event_video_show') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
...
def video_show_profile(request,event_id):
    qry = "with t as( SELECT id event_id, To_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created, ( SELECT username FROM auth_user WHERE id = user_id limit 1) username , ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/district')) district , ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/upazila')) upazila , COALESCE( ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/union')),'') union_name , json->>'geo/para_bazar_school' para_bazar_school , json->>'geo/village' village FROM logger_instance WHERE xform_id = ( SELECT id FROM logger_xform WHERE id_string = 'event_video_show') AND deleted_at IS NULL AND id = "+ str(event_id)+"), t1 as ( select json->>'event_id' event_id, json->>'participant/male_greater_equal_18' male_greater_equal_18 , json->>'participant/male_less_18' male_less_18 , json->>'participant/female_greater_equal_18' female_greater_equal_18 , json->>'participant/female_less_18' female_less_18 , json->>'participant/total_participant' total_participant , CASE WHEN ( json->>'participant/participant_category')::int = 1 THEN 'Aspirant Migrants' WHEN ( json->>'participant/participant_category')::int = 2 THEN 'Returnee' WHEN ( json->>'participant/participant_category')::int = 3 THEN 'Community Members' WHEN ( json->>'participant/participant_category')::int = 4 THEN 'Local Governments' WHEN ( json->>'participant/participant_category')::int = 5 THEN 'National Government' WHEN ( json->>'participant/participant_category')::int = 6 THEN 'Service Provider' WHEN ( json->>'participant/participant_category')::int = 99 THEN (json->>'participant/participant_category_other') END participant_category, json->>'remarks' remarks FROM logger_instance WHERE xform_id = ( SELECT id FROM logger_xform WHERE id_string = 'event_attendance_video_show') AND deleted_at IS NULL and (json->>'event_id')::int = "+ str(event_id)+" )select t.event_id,t.date_created ,t.username,t.district ,t.upazila,t.union_name ,t.para_bazar_school,t.village ,t1.male_greater_equal_18, t1.male_less_18, t1.female_greater_equal_18, t1.female_less_18, t1.total_participant, t1.participant_category, t1.remarks from t left join t1 on t.event_id = t1.event_id::int"
    df = pandas.read_sql(qry,connection)
    event_id = df.event_id.tolist()[0] if len(df.event_id.tolist()) and df.event_id.tolist()[0] is not None  else ''
    date_created = df.date_created.tolist()[0] if len(df.date_created.tolist()) and df.date_created.tolist()[0] is not None  else ''
    submitted_by = df.username.tolist()[0] if len(df.username.tolist()) and df.username.tolist()[0] is not None  else ''
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{300,301,302}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def get_school_quiz_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'school_quiz_checklist') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz_review') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1) then 1 else 0 end review from logger_instance st where xform_id = (select id from logger_xform where id_string = 'event_school_quiz') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    elif role == 'RSC Manager':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'school_quiz_checklist') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_school_quiz_review') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1) then 1 else 0 end review from logger_instance st where xform_id = (select id from logger_xform where id_string = 'event_school_quiz') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
...
def school_quiz_profile(request,event_id):
    qry = "with t as( SELECT id event_id, To_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created, ( SELECT username FROM auth_user WHERE id = user_id limit 1) username , ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/district')) district , ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/upazila')) upazila , COALESCE( ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/union')),'') union_name , json->>'geo/para_bazar_school' para_bazar_school , json->>'geo/village' village FROM logger_instance WHERE xform_id = ( SELECT id FROM logger_xform WHERE id_string = 'event_school_quiz') AND deleted_at IS NULL AND id = "+ str(event_id)+"), t1 as ( select json->>'event_id' event_id, json->>'participant/male_11_14' male_11_14 , json->>'participant/male_15_16' male_15_16 , json->>'participant/female_11_14' female_11_14 , json->>'participant/female_15_16' female_15_16 , json->>'participant/total_participant' total_participant , CASE WHEN ( json->>'participant/participant_category')::int = 1 THEN 'Aspirant Migrants' WHEN ( json->>'participant/participant_category')::int = 2 THEN 'Returnee' WHEN ( json->>'participant/participant_category')::int = 3 THEN 'Community Members' WHEN ( json->>'participant/participant_category')::int = 4 THEN 'Local Governments' WHEN ( json->>'participant/participant_category')::int = 5 THEN 'National Government' WHEN ( json->>'participant/participant_category')::int = 6 THEN 'Service Provider' WHEN ( json->>'participant/participant_category')::int = 99 THEN (json->>'participant/participant_category_other') END participant_category, json->>'remarks' remarks FROM logger_instance WHERE xform_id = ( SELECT id FROM logger_xform WHERE id_string = 'event_attendance_school_quiz') AND deleted_at IS NULL and (json->>'event_id')::int = "+ str(event_id)+" )select t.event_id,t.date_created ,t.username,t.district ,t.upazila,t.union_name ,t.para_bazar_school,t.village ,t1.male_11_14, t1.male_15_16, t1.female_11_14, t1.female_15_16, t1.total_participant, t1.participant_category, t1.remarks from t left join t1 on t.event_id = t1.event_id::int"
    df = pandas.read_sql(qry,connection)
    event_id = df.event_id.tolist()[0] if len(df.event_id.tolist()) and df.event_id.tolist()[0] is not None  else ''
    date_created = df.date_created.tolist()[0] if len(df.date_created.tolist()) and df.date_created.tolist()[0] is not None  else ''
    submitted_by = df.username.tolist()[0] if len(df.username.tolist()) and df.username.tolist()[0] is not None  else ''
    ...
    village = df.village.tolist()[0] if len(df.village.tolist()) and df.village.tolist()[0] is not None else ''
    para_bazar_school = df.para_bazar_school.tolist()[0] if len(df.para_bazar_school.tolist()) and df.para_bazar_school.tolist()[0] is not None  else ''
    male_11_14 = df.male_11_14.tolist()[0] if len(df.male_11_14.tolist()) and df.male_11_14.tolist()[0] is not None else ''
    male_15_16 = df.male_15_16.tolist()[0] if len(df.male_15_16.tolist()) and df.male_15_16.tolist()[0] is not None  else ''
    female_11_14 = df.female_11_14.tolist()[0] if len(df.female_11_14.tolist()) and df.female_11_14.tolist()[0] is not None  else ''
    female_15_16 = df.female_15_16.tolist()[0] if len(df.female_15_16.tolist()) and df.female_15_16.tolist()[0] is not None  else ''
    total_participant = df.total_participant.tolist()[0] if len(df.total_participant.tolist()) and df.total_participant.tolist()[0] is not None  else ''
    participant_category = df.participant_category.tolist()[0] if len(df.participant_category.tolist()) and df.participant_category.tolist()[0] is not None  else ''


    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{400,401,402}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
...
def get_tea_stall_meeting_list(request):
    ...
    role = __db_fetch_single_value(
        "select (SELECT role FROM public.usermodule_organizationrole WHERE id = role_id limit 1)role_name  from usermodule_userrolemap where user_id = (select id from usermodule_usermoduleprofile where user_id= " + str(
            user_id) + ")")
    if role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_checklist') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_review') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1) then 1 else 0 end review from logger_instance st where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting') and deleted_at is null and user_id="+str(user_id)+")select * from t"
    elif role == 'Field Officer':
        query = "with t as( select ROW_NUMBER() OVER (ORDER BY id) AS serial_no,id,to_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created,(select username from auth_user where id = user_id limit 1) username , (select field_name from geo_data where geocode = (json->>'geo/district')) district ,(select field_name from geo_data where geocode = (json->>'geo/upazila')) upazila , coalesce((select field_name from geo_data where geocode = (json->>'geo/union')),'') union_name ,json->>'geo/para_bazar_school' para_bazar_school ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_checklist') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1 ) then 1 else 0 end observation ,case when id::text = (select (json->>'event_id')::text from logger_instance where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting_review') and deleted_at is null and (json->>'event_id') is not null and (json->>'event_id')::int = st.id limit 1) then 1 else 0 end review from logger_instance st where xform_id = (select id from logger_xform where id_string = 'event_tea_stall_meeting') and deleted_at is null and user_id=any(select user_id from usermodule_usermoduleprofile where rsc_name_id = any(select rsc_name_id from usermodule_usermoduleprofile where user_id ="+str(user_id)+")))select * from t"
    else:
...
def tea_stall_meeting_profile(request,event_id):
    qry = "with t as( SELECT id event_id, To_char((json->>'event/event_start_time')::date,'DD/MM/YYYY') date_created, ( SELECT username FROM auth_user WHERE id = user_id limit 1) username , ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/district')) district , ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/upazila')) upazila , COALESCE( ( SELECT field_name FROM geo_data WHERE geocode = (json->>'geo/union')),'') union_name , json->>'geo/para_bazar_school' para_bazar_school , json->>'geo/village' village FROM logger_instance WHERE xform_id = ( SELECT id FROM logger_xform WHERE id_string = 'event_tea_stall_meeting') AND deleted_at IS NULL AND id = "+ str(event_id)+"), t1 as ( select json->>'event_id' event_id, json->>'participant/male_greater_equal_18' male_greater_equal_18 , json->>'participant/male_less_18' male_less_18 , json->>'participant/female_greater_equal_18' female_greater_equal_18 , json->>'participant/female_less_18' female_less_18 , json->>'participant/total_participant' total_participant , CASE WHEN ( json->>'participant/participant_category')::int = 1 THEN 'Aspirant Migrants' WHEN ( json->>'participant/participant_category')::int = 2 THEN 'Returnee' WHEN ( json->>'participant/participant_category')::int = 3 THEN 'Community Members' WHEN ( json->>'participant/participant_category')::int = 4 THEN 'Local Governments' WHEN ( json->>'participant/participant_category')::int = 5 THEN 'National Government' WHEN ( json->>'participant/participant_category')::int = 6 THEN 'Service Provider' WHEN ( json->>'participant/participant_category')::int = 99 THEN (json->>'participant/participant_category_other') END participant_category, json->>'remarks' remarks FROM logger_instance WHERE xform_id = ( SELECT id FROM logger_xform WHERE id_string = 'event_attendance_tea_stall_meeting') AND deleted_at IS NULL and (json->>'event_id')::int = "+ str(event_id)+" )select t.event_id,t.date_created ,t.username,t.district ,t.upazila,t.union_name ,t.para_bazar_school,t.village ,t1.male_greater_equal_18, t1.male_less_18, t1.female_greater_equal_18, t1.female_less_18, t1.total_participant, t1.participant_category, t1.remarks from t left join t1 on t.event_id = t1.event_id::int"
    df = pandas.read_sql(qry,connection)
    event_id = df.event_id.tolist()[0] if len(df.event_id.tolist()) and df.event_id.tolist()[0] is not None  else ''
    date_created = df.date_created.tolist()[0] if len(df.date_created.tolist()) and df.date_created.tolist()[0] is not None  else ''
    submitted_by = df.username.tolist()[0] if len(df.username.tolist()) and df.username.tolist()[0] is not None  else ''
    ...
    user_id = request.user.id
    query = """ SELECT distinct category_id,'<div class="row"> <div class="col-lg-12"> <div class="panel-group"  role="tablist" aria-multiselectable="true"><div class="panel panel-default" style="margin-bottom: 10px;"><div style="height: 48px;" class="panel-heading" role="tab" id="heading'||category_id||'"><h4 class="panel-title"><a style="font-weight: bold;" class="collapsed"  onclick="load_forms('|| category_id ||',''internal_accordian'|| category_id ||''')" role="button" data-toggle="collapse"  href="#collapse'|| category_id ||'" aria-expanded="false" aria-controls="collapse'|| category_id ||'"> ' ||(SELECT category_name FROM forms_categories WHERE id = fc.category_id :: INT) || ' </a>'|| case when first_value(can_submit)over(PARTITION by category_id ORDER by can_submit desc) = 1 then '<a onclick="load_forms_list('|| category_id ||')"  class="btn btn-success btn-sm pull-right"   id="form'|| category_id ||'"  data-toggle="modal" data-target="#myModal"  ><i class="fa fa-4x fa fa-plus"></i></a>' else '' end  ||' </h4></div><div id="collapse'|| category_id ||'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'|| category_id ||'"><div class="panel-body"><div class="panel-group" id="internal_accordian'|| category_id ||'" role="tablist" aria-multiselectable="true"></div></div></div></div></div></div></div>' as form_str FROM vwrolewiseformpermission rf, forms_categories_relation fc WHERE ( rf.can_view = 1 OR rf.can_submit = 1) AND fc.form_id = rf.xform_id and fc.category_id = any('{200,201,202}') AND user_id = """ + str(user_id) + """ order by category_id asc """
    df = pandas.DataFrame()
    df = pandas.read_sql(query, connection)
    main_str = ""
    for each in df['form_str']:
