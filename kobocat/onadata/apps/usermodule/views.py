...
def user_login(request):
    ...
    # This scenario would most likely be a HTTP GET.
        ...
        home = "login"
        return render_to_response('usermodule/login.html', {'redirect_url':redirect_url,'home':home}, context)
...
def catchment_tree_test(request,user_id):
    ...
    query = "select (SELECT  organization FROM public.usermodule_organizations where id = (select organisation_name_id from usermodule_usermoduleprofile where user_id = " + str(
        user_id) + ")) ,(select designation from usermodule_usermoduleprofile where user_id = " + str(
...
'''
def custom_export(request, username, id_string, export_type):
    owner = get_object_or_404(User, username__iexact=username)
    xform = get_object_or_404(XForm, id_string__exact=id_string, user=owner)
    if not has_permission(xform, owner, request):
        return HttpResponseForbidden(_(u'Not shared.'))
    query = request.POST.get("query")
    # {"$and" : [ {"_submission_time":{"$gte":"2019-08-24T00:00:00","$lte":"2019-09-24T23:59:59"}},{"_submitted_by":"fo1234"} ] }
    print query
    export_type = 'xls'
    force_xlsx = true
    options = {
    ...
    print  "check 7_________________________________________"
