...
class DashboardGenerator(models.Model):
    ...
    parent_chart = models.IntegerField()
    content_row = models.IntegerField()
    def __str__(self):
...
class DashboardControlsCascaded(models.Model):
    cascaded_parent =models.ForeignKey('DashboardControlsGenerator',related_name='dashboard_cascaded_parent', blank=True, null=True)
    cascaded_child =  models.ForeignKey('DashboardControlsGenerator',related_name='dashboard_cascaded_child', blank=True, null=True)
    parent_key = models.CharField(max_length=100, blank=True)
    def __str__(self):
        return str(self.cascaded_parent)

    class Meta:
        managed = False

        db_table = 'dashboard_controls_cascaded'
