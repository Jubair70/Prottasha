...
import json
from onadata.apps.dashboard.utility_functions import *
import pandas as pd
...
class HighchartsConfig():
    ...
    def generate_json_bar_line_area_Chart(self, name_field, category_field, data_field, df):
        ...
        if "name_sorting" in df and "category_sorting" in df and "point_placement" in df:
                #print df
                df = pd.pivot_table(df, values=data_field, index=['name_sorting', name_field,'point_placement'],

                                    columns=["category_sorting", category_field])
                ...
                for row in df.iterrows():
                    ...
                    dict['pointPadding'] = float(index[2])

                    seriesData.append(dict)
        elif "name_sorting" in df and "category_sorting" in df:
            df = pd.pivot_table(df, values=data_field, index=['name_sorting', name_field],
                                columns=["category_sorting", category_field])
            df = df.fillna(0)
            # Check in Name Sorting order defined or not
            appearance = json.loads(self.dashboardGenerator.chart_object)
        ...
        **Generated graph of this here http://jsfiddle.net/gh/get/library/pure/highcharts/highcharts/tree/master/samples/highcharts/demo/column-basic/
        ...
        if df.empty == False:
            if "colors" in df and "name_sorting" in df and "category_sorting" in df:
                df = pd.pivot_table(df, values=data_field, index=['name_sorting', name_field, 'colors'],
                                    columns=["category_sorting", category_field])
                print df
                df = df.fillna(0)
                # Check in Name Sorting order defined or not
                ...
                for row in df.iterrows():
                    ...
                    seriesData.append(dict)
                print seriesData
            elif "name_sorting" in df and "category_sorting" in df and "point_placement" in df:
                #print df
                df = pd.pivot_table(df, values=data_field, index=['name_sorting', name_field,'point_placement'],
    ...
    def generate_json_pie_chart(self,  name_field, category_field, data_field, dataframe):
        """
        Pie Chart
        :param name_field:
        :param category_field:
        ...
        if "innersize" in appearance:
            innersize = appearance["innersize"]
        print "####################################"
        print "pie"
        jsonForChart = json.dumps({'seriesdata': [{'name': 'count','type': 'pie', 'innerSize':innersize,'data': seriesData}]},
